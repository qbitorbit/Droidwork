...
NOTE: This client is configured for Confluence Server/Data Center (self-hosted),
NOT Confluence Cloud. URL formats differ between the two.

DUAL URL SUPPORT:
- CONFLUENCE_BASE_URL: Used for API calls (e.g., https://confluence.rnd.internal:8444)
- CONFLUENCE_BROWSER_URL: Used for clickable links (e.g., https://confluence:8444)
"""
...
class ConfluenceClient:
    ...
    NOTE: Configured for Confluence Server/Data Center (self-hosted).
    
    Environment Variables:
    - CONFLUENCE_BASE_URL: API endpoint (e.g., https://confluence.rnd.internal:8444)
    - CONFLUENCE_BROWSER_URL: Browser links (e.g., https://confluence:8444)
      Falls back to CONFLUENCE_BASE_URL if not set.
    ...
    def __init__(self):
        ...
        # Browser URL for clickable links (falls back to base_url if not set)
        self.browser_url = os.getenv("CONFLUENCE_BROWSER_URL", self.base_url).rstrip("/")
    ...
    # =========================================================================
    # URL HELPERS (Confluence Server/Data Center format)
    # =========================================================================
    
    def _build_page_url(self, page_id: str) -> str:
        """
        Build a clickable page URL for Confluence Server/Data Center.
        
        Uses CONFLUENCE_BROWSER_URL for user-facing links.
        Format: {browser_url}/pages/viewpage.action?pageId={page_id}
        """
        return f"{self.browser_url}/pages/viewpage.action?pageId={page_id}"
    
    def _build_space_url(self, space_key: str) -> str:
        """
        Build a clickable space URL for Confluence Server/Data Center.
        
        Uses CONFLUENCE_BROWSER_URL for user-facing links.
        Format: {browser_url}/display/{space_key}
        """
        return f"{self.browser_url}/display/{space_key}"
        ...
        return [
            {
                ...
                "name": space.get("name", ""),
                "type": space.get("type", ""),
                "url": self._build_space_url(space.get("key", ""))
            }
    ...
    def get_page(
        ...
        image_urls = self._extract_image_urls(body_html, retrieved_page_id)
        ...
        return PageContent(
            ...
            body_text=body_text,
            body_html=body_html,
            url=self._build_page_url(retrieved_page_id),
        ...
        return [
            {
                ...
                "url": self._build_page_url(child.get("id", ""))
            }
    ...
    def create_page(
        ...
        return {
            ...
            "url": self._build_page_url(created_page_id),
    ...
    def update_page(
        ...
        updated_page_id = result.get("id", "")
        
        return {
            "page_id": updated_page_id,
            "title": result.get("title", ""),
            "url": self._build_page_url(updated_page_id),
    ...
    def _download_image_base64(self, img_url: str) -> Dict[str, str]:
        ...
        # Handle relative URLs - use base_url (API) for downloading
    ...
    def _extract_image_urls(self, html: str, page_id: str) -> List[str]:
        ...
        for ac_img in soup.find_all("ac:image"):
            ...
            if ri_att:
                ...
                if filename:
                    # Use download/attachments path for Server/DC
                    urls.append(f"/download/attachments/{page_id}/{filename}")
