...
class ConfluenceClient:
    ...
    URL format: {base_url}/pages/viewpage.action?pageId={id}
    """
    ...
    def _build_page_url(self, page_id: str) -> str:
        ...
        Format: {base_url}/pages/viewpage.action?pageId={page_id}
        This format works universally for Server/DC installations.
        """
    ...
    def _build_space_url(self, space_key: str) -> str:
        ...
        """
        return f"{self.base_url}/display/{space_key}"
    ...
    def search(
        ...
        for item in results.get("results", []):
            ...
            items.append(SearchResult(
                ...
                space_key=content.get("space", {}).get("key", "") if isinstance(content.get("space"), dict) else "",
                space_name=content.get("space", {}).get("name", "") if isinstance(content.get("space"), dict) else "",
                url=self._build_page_url(page_id),  # FIXED: Use helper
                excerpt=item.get("excerpt", ""),
                last_modified=content.get("version", {}).get("when", "") if isinstance(content.get("version"), dict) else ""
            ))
        ...
        return [
            {
                ...
                "name": space.get("name", ""),
                "type": space.get("type", ""),
                "url": self._build_space_url(space.get("key", ""))  # FIXED: Use helper
            }
            for space in spaces.get("results", [])
        ]
    ...
    def get_page(
        ...
        return PageContent(
            ...
            body_text=body_text,
            body_html=body_html,
            url=self._build_page_url(retrieved_page_id),  # FIXED: Use helper
            version=page.get("version", {}).get("number", 1),
            last_modified=page.get("version", {}).get("when", ""),
            last_modifier=page.get("version", {}).get("by", {}).get("displayName", ""),
            image_urls=image_urls,
        ...
        return [
            {
                "page_id": child.get("id", ""),
                "title": child.get("title", ""),
                "url": self._build_page_url(child.get("id", ""))  # FIXED: Use helper
            }
            for child in children.get("results", [])
        ]
    ...
    def create_page(
        ...
        return {
            ...
            "title": result.get("title", ""),
            "url": self._build_page_url(created_page_id),  # FIXED: Use helper
            "message": f"Page '{title}' created successfully"
        }
    ...
    def update_page(
        ...
        return {
            ...
            "title": result.get("title", ""),
            "url": self._build_page_url(updated_page_id),  # FIXED: Use helper
            "version": result.get("version", {}).get("number", 0),
            "message": f"Page '{title}' updated successfully"
        }
